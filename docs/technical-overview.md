...
## フォーカスモードでもtextareaを作り直さない

フォーカスモードは別画面やモーダルではなく、`App.vue`のレイアウト状態を切り替えて実現している。PCでは画面を25%／50%／25%へ固定し、中央に選択中のタブ名とMarkdown入力欄、左に終了操作と入力支援の解説を配置する。767px以下では中央を全幅にし、解説を左ドロワーとして重ねる。

重要なのは、通常モードとフォーカスモードで`MarkdownEditor`と内部のtextareaを共有している点である。モード切り替え時にtextareaを別コンポーネントへ差し替えると、カーソル、選択範囲、スクロール位置だけでなく、ブラウザが管理するUndo/Redo履歴も失われる可能性がある。そのため、通常のタブ一覧とフォーカス用のタブ名だけを切り替え、textareaはマウントしたままCSSグリッドを変更する。

`MarkdownEditor`は選択範囲と`scrollTop`／`scrollLeft`を取得・復元する小さな操作を公開し、`useFocusMode`がレイアウト変更の前後で呼び出す。切り替え後は同じtextareaへフォーカスするため、直前の位置からすぐに入力を続けられる。通常設定で内部スクロールがOFFでも、フォーカスモード中だけはtextarea内部スクロールを有効にするが、保存済み設定値は変更しない。

タブ名編集は`EditableTabName`へ共通化し、通常タブとフォーカスモードの双方で同じ確定・キャンセル規則を使う。Escapeキーは通常モードとフォーカスモードのトグルにも使用するが、名前編集中はイベント伝播を止め、設定、警告、情報モーダルなどが処理した場合も切り替えない。`useFocusMode`は各UIのキー処理が終わった後に`defaultPrevented`を確認し、未処理のEscapeだけをモード切り替えとして扱う。

フォーカスモードと解説欄の開閉状態は一時的なUI状態であり、LocalStorageへ保存しない。再読み込み時は常に通常モードから開始し、既存のタブ内容とアプリ設定だけを従来の保存処理から復元する。

## LocalStorage障害を通常系から切り離す

Markdown入力とテーマは変更から500ミリ秒後に保存する。入力のたびに同期的なStorage書き込みを行わず、最後の変更をまとめるためである。タブ追加・切り替え・並べ替え・名称変更・削除・復元・完全削除、変換形式、アプリ設定は、変更直後の再読み込みでも状態を失わないよう即時保存している。
...
Markdownタブの表示順は `EditorState.tabs` の配列順を正とし、並べ替え操作ではドラッグ元IDとドロップ先IDからその時点の挿入位置を求める。タブオブジェクトや選択中IDは変更せず、並べ替えた配列を操作直後に保存する。専用の順序フィールドと保存形式の新版を追加しないため、既存のversion 1データは保存済みの配列順のまま復元できる。
...
| 入力置換設定 | `md-converter:input-replacements:v1` |

アプリ設定にはエディター内部スクロールのON/OFF、PC表示の左右ペイン比率、全角Markdown補正のON/OFFを保存する。旧保存値に比率がない場合や、保存比率が20〜80%の範囲外の場合は50:50へフォールバックし、全角Markdown補正がない場合や不正な型の場合はOFFへフォールバックする。区切りバーの操作時はworkspaceの実幅から各ペイン280px以上となる可動範囲を求め、ポインター操作とキーボード操作で同じ比率更新経路を利用する。767px以下では既存の入力・右ペイン切り替えを優先し、区切りバーを表示しない。
...
入力置換も同じ境界へ統合している。直接入力ではSpace・全角Space・Enterの直前、IMEでは`compositionend`で確定した範囲全体だけを判定し、貼り付け、検索置換、Undo／Redoなどの外部更新では実行しない。Enterは置換後の文字列へ既存のリスト継続を適用してから、textareaへ1回の編集として反映する。

ルール登録時は既存と同じremark/GFMパーサーで単独文書を解析し、通常テキスト以外のノードを生成する値を拒否する。`**`やバッククォート単体など、不完全な記法として通常テキストになる記号だけは明示禁止で補う。本文への適用時はmdastの開始・終了offsetを使ってインラインコードとコードブロックを除外するため、Markdown全文を正規表現で判定しない。

全角Markdown補正も`useMarkdownEditor`へ接続し、設定がONの場合だけ、直接入力またはIME確定した全角Markdown記号を1文字から半角化する。ブロック構文用の記号、全角数字、区切り空白は行頭のMarkdown候補位置だけを補正し、本文全体のNFKC正規化は行わない。既存のmdast位置情報からコード範囲を取得してコード本文を保護し、全角コードフェンスだけはコード構造の開始・終了記号として補正する。貼り付け、検索置換、復元、Undo／Redoなどの外部更新は対象外とし、Enterでは全角Markdown補正、入力置換、リスト継続の順に1回の編集結果を計算する。
