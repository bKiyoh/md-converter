...
767px以下の画面では「入力」と「変換結果」のタブを使用します。タブへフォーカスした状態で、左右矢印キー、Homeキー、Endキーでも切り替えられます。Markdown入力とテーマは変更から500ms後、文書タブの追加・切り替え・並べ替え・名前変更・削除・復元、変換形式、エディター内部スクロール設定、左右ペイン比率、入力置換設定は変更直後に次のキーへ保存されます。
