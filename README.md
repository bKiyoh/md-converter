...
767px以下の画面では「入力」と「変換結果」のタブを使用します。タブへフォーカスした状態で、左右矢印キー、Homeキー、Endキーでも切り替えられます。Markdown入力とテーマは変更から500ms後、文書タブ操作、変換形式、エディター内部スクロール設定、左右ペイン比率は変更直後に次のキーへ保存されます。
